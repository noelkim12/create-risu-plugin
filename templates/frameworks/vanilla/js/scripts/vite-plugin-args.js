 ...
 *
 * plugin-args.json을 읽어서 src/core/plugin-config.js를 자동 생성합니다.
 * Webpack의 PluginArgsWebpackPlugin과 동일한 기능을 제공합니다.
 */
...
export function vitePluginArgs(options = {}) {
  ...
  return {
    ...
    configureServer(server) {
      // plugin-args.json 변경 감지
      server.watcher.add(argsFilePath);

      server.watcher.on('change', file => {
        if (file === argsFilePath) {
          console.log('📝 plugin-args.json changed, regenerating plugin-config.js...');
          generatePluginConfig();
          // HMR 트리거
  ...
  function generatePluginConfig() {
    try {
      ...
      if (!fs.existsSync(argsFilePath)) {
        console.warn(`⚠️  ${argsFilePath} not found, skipping plugin-config.js generation`);
        return;
      }
      ...
      // PluginArgs 클래스 코드 생성
      const code = generateCode(args);

      // 기존 파일 내용과 비교 (무한 루프 방지)
      ...
      console.log(`✅ Generated plugin-config.js from plugin-args.json`);
    } catch (error) {
      console.error('❌ Failed to generate plugin-config.js:', error);
    }
  }
