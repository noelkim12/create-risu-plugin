...
async function findAvailablePort(startPort = DEFAULT_PORT) {
  ...
  throw new Error(
    `[DevServer] No available ports found in range ${startPort}-${startPort + MAX_PORT_ATTEMPTS - 1}`,
  );
}
...
function broadcastReload() {
  ...
  console.log(
    `📤 [DevServer] Broadcast reload to ${successCount}/${clients.size} clients (${new Date().toLocaleTimeString()})`,
  );
}
