...
export class TemplateComposer {
  ...
  /**
   * ESM 모듈이 필요한 프레임워크인지 확인합니다
   * @returns {boolean} ESM 모듈이 필요하면 true
   */
  requiresESM() {
    // Svelte 및 향후 ESM이 필요한 프레임워크들
    const esmFrameworks = ['svelte'];
    return esmFrameworks.includes(this.config.framework);
  }
  ...
  async copyCommonScripts() {
    ...
    // ESM이 필요한 프레임워크는 modules 폴더에서 복사
    const scriptsSubDir = this.requiresESM() ? 'modules' : '';
    const scriptsSrcDir = scriptsSubDir
      ? path.join(this.templatesBaseDir, 'scripts', scriptsSubDir, languageAbbr)
      : path.join(this.templatesBaseDir, 'scripts', languageAbbr);
    ...
    for (const scriptFile of jsFiles) {
      ...
      showInfo(`공통 스크립트 복사: ${scriptFile}${scriptsSubDir ? ' (ESM)' : ''}`);
    }
  }
